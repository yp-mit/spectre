function y = biawgnPPVbound(N, Pe, x2, x3, x4)

% biawgnPPVbound : Polyanskiy-Poor-Verdu (PPV) metaconverse bound for
%                  binary transmission corrupted by AWGN (bi-AWGN channel)
%
% R = biawgnPPVbound(N,Pe,SNRdB,'approx') returns the upper bound on rate
...
%
%   'normal'  - normal approximation, a O(n^-1) approximation; this is a
%               very fast algorithm, but may provide a weak approximation.
%
%   'On2'     - (default) the O(n^-2) approximation derived from [1]; this
%               is approximately 50 times slower than the 'normal' option,
%               but much more reliable at low Pe.
%
%   'On3'     - the O(n^-3) approximation derived from [1]; this is twice
%               slower than 'On2', and much more unrealiable at high Pe. 
%               It can  be used to identify the region where 'On2' is a 
%               reliable solution, namely the region where 'On2' and 'On3' 
%               closely match.
%
%   'full'    - (this option will be available soon) the full bound derived
%               numerically from [1].
%
% SNRdB = biawgnPPVbound(N,Pe,R,'approx','error') for a codeword of length 
...
elseif strcmp(x4,'error')
    ...
    for k = 1:length(N) % cycle on requests
        ...
        if (n==inf)||strcmp(x3,'normal')            
            ...
            if k==1 
                om0 = 1;
            else % use previous value (hoping it is close to true value)
                om0 = y(k-1); 
            end
            % calculate SNR value
