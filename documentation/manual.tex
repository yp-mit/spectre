...
Giuseppe Durisi, Johan \"Ostman, Yury Polyanskiy, Wei Yang}
...
\end{equation}  
  ...
  The power constraint is defined \textit{per coherence interval}, i.e., each codeword $C$  consists of $\tfdiv$ subcodewords $C_1,\ldots, C_\tfdiv$ that must
  satisfy:
    \begin{equation*}
      \tr\bigl\{C_{k}^H C_{k}\bigr\}= \cohtime\snr,\quad  k=1,\dots,\tfdiv. 
    \end{equation*}
    %
...
\section{Converse helper functions: \texttt{MC\_USTM\_2x2\_q1x2()}, \texttt{MC\_USTM\_2x2\_q\_2x2()}, \texttt{MC\_USTM\_4x4\_q\_Mx4}} % (fold)
...
\section{Converse bound: \texttt{compute\_MC\_2x2\_telatar}}
Format
%
\begin{verbatim}
...
\section{Outage: \texttt{outage\_mi()}, \texttt{outage\_alamouti()}, \texttt{outage\_2x2\_telatar()} } % (fold)
\label{sec:outage}
