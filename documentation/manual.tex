...
\begin{itemize}
  ...
  \begin{equation}\label{eq:bfmimo}
  	\mathbb{Y}_k = X_k \mathbb{H}_k + \mathbb{W}_k\,, \qquad k=1,\ldots, \tfdiv\,,
\end{equation}  
  where $\mathbb{Y}_k, \mathbb{W}_k \in \mathbb{C}^{\cohtime \times \rxant}$, $X_k \in \mathbb{C}^{\cohtime \times
  \txant}$ and $\mathbb{H}_k\in\mathbb{C}^{\txant \times \rxant}$. The noise $\mathbb{W}$ and the channel gain
  $\mathbb{H}$ have independent $\mathcal{CN}(0,1)$ entries.


  The power constraint is defined \textit{per coherence interval.} I.e. each codeword $X_1,\ldots, X_\tfdiv$ should
  satisfy:
    \begin{equation*}
      \tr\bigl\{X_{k}^H X_{k}\bigr\}= \cohtime\snr,\quad  k=1,\dots,\tfdiv. 
    \end{equation*}
    %
...
\section{Achievability bounds: \texttt{DT\_USTM\_NxM()}, \texttt{DT\_USTM\_MxM()}} % (fold)
...
\section{Converse bounds: \texttt{MC\_USTM\_2x2\_q1x2()}, \texttt{MC\_USTM\_2x2\_q\_2x2()}, \texttt{MC\_USTM\_4x4\_q\_Mx4}} % (fold)
...
\section{Alamouti: \texttt{DT\_USTM\_Alamouti()}, \texttt{MC\_USTM\_Alamouti()}} % (fold)
\label{sec:alamouti}
...
\section{Outage: \texttt{outage\_mi()}, \texttt{outage\_alamouti()} } % (fold)
\label{sec:outage}
...
Functions that compute the outage mutual information $I_\epsilon$ for a given set of input covariance matrices and the outage mutual
information with Alamouti:

$$ \PP[ \EE[\imath(X^\tfdiv; \mathbb{Y}^\tfdiv)|\mathbb{H}^\tfdiv] \le
I_\epsilon] = \epsilon\,, $$
where distribution of $X^\tfdiv=(X_1,\ldots,X_\tfdiv)$ is specified by \verb|pow_all| and 
...
\section{Normal approximation: \texttt{normapprox\_simo()}, \texttt{normapprox\_mimo\_iso()}}
