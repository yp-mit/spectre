...
for i=1:samples1 % run time = O(samples1 * samples2 * T)
    ...
    for j=1:samples2
        ...
        for k=1:T
            z = randn(n_r,1);
            y = h*x(:,k) + z;
            info_dens_x(k) = 1/2 * ( log2(det(K)) - norm(y-h*x(:,k))^2 + y'*inv(K)*y );
        end
    ...
    if (mod(i,500)==0)
        current_samples = i
        disp = 1/T * mean(exp_x_samples(1:i))
    end
...
exp_x_samples = zeros(samples1,1);
for i=1:samples1 % run time = O(samples1 * samples2 * T)
    x = sqrt(P/n_t)*sqrt(.5)*(randn(n_t,T) + 1i*randn(n_t,T));
    info_dens_x_samples = zeros(samples2,1);
    ...
    for j=1:samples2
        ...
        for k=1:T
            ...
            y = h*x(:,k) + z;
            info_dens_x(k) = log2(det(K)) - norm(y-h*x(:,k))^2 + y'*inv(K)*y;
        end
        info_dens_x_samples(j) = real(sum(info_dens_x));
    end
    mu_x = mean(info_dens_x_samples);
    exp_x = mean( (info_dens_x_samples - mu_x).^2 );
    exp_x_samples(i) = exp_x;
