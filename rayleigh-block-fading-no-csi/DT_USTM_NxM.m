...
function [R,current_eps]=DT_USTM_NxM(snrdB,T,L,Mt,Mr,epsilon,prec,filename)
%
...
% Mr: number of receive antennas (Mt<Mr); for the square case, use the other function
...
for k = 1:K %do K montecarlo runs
        ...
        for l = 1:L %Create each realization
            Sigma = svd(D*Z(:,:,l)).^2;    
            M = createM(Mt,Mr,T,Sigma,lambda2); %create  matrix
            ...
            i_temp = const- TraceZ  +sum(Sigma) - logdetM + log(vanderTerm); %Information density for time l (i(x_l, y_l)
